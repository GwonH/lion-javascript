...
function createUserCard({
  id='', //값 넘어오지 않았을때 undefined 뜨지 않게 하려고 기본값 빈문자로 설정
  name='',
  email='',
  website=''
}){

  // const {id,name,email,website} = data; //data.id 이렇게 하나하나 쓰지 말고 구조분해 할당으로 간단하게 쓸수도.. 하지만 받는 즉시 분해가 가능하니까 ()안에 넣어줌
...
function createSpinner(size = 100,loadingMessage = '유저 정보를 가져오는 중...📂'){
  ...
  function creatEmptyCard(size = 200,loadingMessage = '알 수 없는 오류가 발생했습니다...😥'){
    return /* html */`
  ...
  //렌더
  export function renderUserCard(target,data){ //target = 어디에. data =뭐를.
    insertLast(target,createUserCard(data)) //'어디에', 뭘 뿌릴지. data전달
