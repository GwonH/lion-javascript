...
const shopOrder = {
  ...
  // reduce는 acc 값을 반환해줘야 쓸수있음! return값이 필수임. (map도. return 없으면 쓰는 이유가 없음(화살표 함수로 바꾸면 return같이 제거됨!). forEach는 return쓸 필요가 없고.)
  totalPrice(){
  ...
  // totalPrice:function(){
  //   console.log(this);             //이걸 consise method로 바꾸기
  // }
  // totalPrice : () => {   //애로우펑션. this 는 윈도우가 나온다.
  //   console.log(this);
  // }
//   totalPrice(){
//     let total = 0;
...
// console.log(shopOrder.menu[0].price + shopOrder.menu[1].price);
// console.log(shopOrder.menu[0].price * shopOrder.menu[0].count + shopOrder.menu[1].price);


// let totalCost = (shopOrder.menu[0].price * shopOrder.menu[0].count) + (shopOrder.menu[1].price * shopOrder.menu[1].count)


// console.log(totalCost);

// forEach 배열을 순회해서 다 찍어주는 역할을 함. item이 배열의 객체 하나임. 
// let total = 0;
